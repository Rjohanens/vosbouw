<article class="relative isolate group flex flex-col justify-end overflow-hidden rounded-2xl bg-zinc-900 px-8 pb-8 pt-80 sm:pt-48 lg:pt-80 shadow-md">
    <img src="{{ $service->thumbnail()?->getUrl() }}" alt="{{ $service->name }}" class="absolute inset-0 -z-10 h-full w-full object-cover group-hover:scale-105 transition duration-300">
    ...
    <h3 class="mt-3 text-lg font-semibold leading-6 text-white">
      <a href="{{ route('service.show', $service) }}" class="flex space-x-2 justify-between items-center">
